...
typedef struct {
  ...
  uint8_t *pdu;
  /// The payload + CRC size in bits, "B" from 36-212   载荷+CRC的总bit数
  uint32_t B;
  /// Pointer to the payload   指向载荷
  uint8_t *b;
  /// Pointers to transport block segments  一个二位数组，表示分段的结果
  ...
  uint8_t *e;
  /// LDPC-code outputs   编码输出
  uint8_t *d[MAX_NUM_NR_DLSCH_SEGMENTS];
  /// Interleaver outputs  交织输出
  uint8_t *f;
  /// Number of code segments   码快的数量
  uint32_t C;
